...
def main(argv):
  ...
  trockOrderFile = curDirectoryPath + "test_orders_trockAT.csv";
  ...
  for test in range(1, testNumber + 1):
    ...
    genParamFile(paramFilePath, genPathOutPut, trockOrderFile, trockLogFile, genReturnsInCalc, genThreshold, startStr, endStr);

    # Call trockAT program to parse input
    subprocess.call([trockATPath, paramFilePath])
    ...
    #Delete unneed files
    os.remove(trockOrderFile);
    os.remove(testerOrderFile);
    os.remove(trockLogFile);
    os.remove(genPathOutPut);
...
def genParamFile(outputParamFilePath, inputCSVPath, outputCSVPath, outputLogFilePath, returnsInCalculation, threshold, startDate, endDate):
