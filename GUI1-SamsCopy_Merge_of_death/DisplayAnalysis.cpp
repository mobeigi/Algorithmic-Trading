...
void DisplayAnalysis::displayReturnsAnalysis(std::AnalysisData *data) {
    ...
    ui->returnsTableDisplay->setColumnCount(4);
    QTableWidgetItem* qtwi = new QTableWidgetItem(QString("Date"),QTableWidgetItem::Type);
    ui->returnsTableDisplay->setHorizontalHeaderItem(0,qtwi);
    qtwi = new QTableWidgetItem(QString("Buy/Sell Pair"),QTableWidgetItem::Type);
    ui->returnsTableDisplay->setHorizontalHeaderItem(1,qtwi);
    qtwi = new QTableWidgetItem(QString("Return Calculation"),QTableWidgetItem::Type);
    ui->returnsTableDisplay->setHorizontalHeaderItem(2,qtwi);
    qtwi = new QTableWidgetItem(QString("Return Value"),QTableWidgetItem::Type);
    ui->returnsTableDisplay->setHorizontalHeaderItem(3,qtwi);
    ...
    for (int i = 0; i < data->tradeDataAvailable(); i++) {
        ...
        if (buy_trades.size() > 0 && sell_trades.size() > 0) {
            ...
            //change this stuff etc
            insertRowIntoReturnsAnalysis("date", "buy sell", "return calc", "retval");
    ...
    insertRowIntoReturnsAnalysis("TOTAL GAIN/LOSS", "", "", "a num");

    printf("FINAL NET RETURN: %.2lf\n",net_return_perc);

}

void DisplayAnalysis::insertRowIntoReturnsAnalysis(std::string date,
                                                   std::string buySellPair,
                                                    std::string returnCalc,
                                                   std::string returnValue) {

    int currRows = ui->returnsTableDisplay->rowCount();
    ui->returnsTableDisplay->setRowCount(currRows + 1);

    QTableWidgetItem* col0 = new QTableWidgetItem(QString::fromStdString(date), QTableWidgetItem::Type);
    col0->setFlags(col0->flags() ^ Qt::ItemIsEditable);
    ui->returnsTableDisplay->setItem(currRows, 0, col0);

    QTableWidgetItem* col1 = new QTableWidgetItem(QString::fromStdString(buySellPair), QTableWidgetItem::Type);
    col1->setFlags(col1->flags() ^ Qt::ItemIsEditable);
    ui->returnsTableDisplay->setItem(currRows, 1, col1);

    QTableWidgetItem* col2 = new QTableWidgetItem(QString::fromStdString(returnCalc), QTableWidgetItem::Type);
    col2->setFlags(col2->flags() ^ Qt::ItemIsEditable);
    ui->returnsTableDisplay->setItem(currRows, 2, col2);

    QTableWidgetItem* col3 = new QTableWidgetItem(QString::fromStdString(returnValue), QTableWidgetItem::Type);
    col3->setFlags(col3->flags() ^ Qt::ItemIsEditable);
    ui->returnsTableDisplay->setItem(currRows, 3, col3);
}
